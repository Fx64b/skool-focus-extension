export default defineContentScript({
  ...
  main() {
    ...
    function toggleElements(selectors: string[], shouldHide: boolean) {
      selectors.forEach((selector) => {
        const elements = document.querySelectorAll(selector);
        elements.forEach((element) => {
          (element as HTMLElement).style.opacity = shouldHide ? '0' : '1';
          (element as HTMLElement).style.pointerEvents = shouldHide ? 'none' : 'auto';
        });
      });
    }
